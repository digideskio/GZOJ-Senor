<?php
 ...
 if (!isset($_SESSION['user_id'])){
	$view_errors= "<a href=loginpage.php>请先登录</a>";
	require("error.php");
	exit(0);
}
if (isset($_GET['id'])){
...
<div class="grid">
	...
	<form id="frmSolution" action="submit.php" method="post" onsubmit="return checksource(document.getElementById('source').value);">
		...
		<div class="row">
			...
			<div class="span7">
		<?php if (isset($id)){?>
			<h2>Problem <span class="fg-color-blue"><strong><?php echo $id?></strong></span></h2>
			<input id=problem_id type='hidden'  value='<?php echo $id?>' name="id" ><br>
		<?php }else{
			$PID="ABCDEFGHIJKLMNOPQRSTUVWXYZ";
			if ($pid>25) $pid=25;
			?>
			<h2>Problem <span class="fg-color-blue"><strong><?php echo $PID[$pid]?></strong></span> of Contest <span class="fg-color-blue"><strong><?php echo $cid?></strong></span><br></h2>
		...
		<?php }?>
			...
			<div class="span2">
				<span class="input-control select">
		...
		<div class="row">
			...
			<div class="input-control textarea span9" style="margin-left:auto !important;margin-right:auto">
				<textarea id="source" name="source" rows="20"><?php echo $view_src?></textarea>
			</div>
		</div>
	<br>
	<center>
		<input id=Submit type="submit" value="提交" onclick=do_submit();/>
	</center>
	</form>
</div></div></div>
<script>
