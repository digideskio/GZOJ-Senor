...
?>
	...
	<form method=POST >
	<p align=center><font size=4 color=#333399>添加一个比赛</font></p>
	<p align=left>竞赛标题:<input class=input-xxlarge  type=text name=title size=71 value="<?php echo isset($title)?$title:""?>"></p>
	...
	是否公开:<select name=private><option value=0>公开</option><option value=1>内部，不公开</option></select>
    <div style="display:none">
	允许使用的语言:<select name="lang[]" multiple="multiple"    style="height:70px">
	<?php
...
?><br /><div style="background-color:rgba(255,102,204,0.1)">
	<Br />对于内部，不公开的比赛，添加要参赛的选手用户名:<textarea style="vertical-align:text-top" name="ulist" rows="20" cols="20"></textarea>
	<br />
	*每行一个，可以将学生学号从Excel整列复制过来，然后要求他们用学号做UserID注册,就能进入Private的比赛作为作业和测验。</div>
	<p><input type=submit value='提 交' name=submit style="width:100px;height:30px;"></p>
	</form>
<?php 
}
