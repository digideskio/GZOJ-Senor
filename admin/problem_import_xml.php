...
if ($_FILES ["fps"] ["error"] > 0) {
	echo "Error: " . $_FILES ["fps"] ["error"] . "文件大小太大，设置最大上传文件大小：upload_max_filesize 和最大HTTP POST请求大小：post_max_size <br><br>";
} else {
...
//		$test_output = getValue ( $searchNode, 'test_output' );
       ...
       // }
			...
			if($spj)
			{
				echo '<script>alert("对不起，题目['.$title.']不能添加，因为现在本OJ不支持SPJ（特殊判断）的题目；NOI系列比赛中也没有SPJ。");</script>';
				;
			}
			/*
			if(!isset($OJ_SAE)||!$OJ_SAE){
			...
			echo "<br><span class=red>题目：$title 已经存在</span>";		
		}
